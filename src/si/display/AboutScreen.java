...
public class AboutScreen implements Screen {
    ...
    public void paint() {
        ...
        gc.fillText("Space Invader Controls", BouncyAsteroidsGame.SCREEN_WIDTH / 2, 64);
        gc.setTextAlign(TextAlignment.LEFT);
        gc.setTextBaseline(VPos.CENTER);
        gc.setFont(new Font("Arial", 20));
        int start = 128;
        int gap = 48;
        gc.fillText("Move Left", 1 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 0 * gap);
        gc.fillText("left arrow", 4 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 0 * gap);
        gc.fillText("Move Right", 1 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 1 * gap);
        gc.fillText("right arrow", 4 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 1 * gap);
        gc.fillText("Fire", 1 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 2 * gap);
        gc.fillText("space bar", 4 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 2 * gap);
        gc.fillText("Play/Pause", 1 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 3 * gap);
        gc.fillText("p", 4 * BouncyAsteroidsGame.SCREEN_WIDTH / 6, start + 3 * gap);
        gc.setTextAlign(TextAlignment.CENTER);
        gc.setFont(new Font("Arial", 28));
